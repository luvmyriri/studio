...
const ResourceItem = ({ item }: { item: ResourceContent }) => {
  if (item.type === 'h3') {
    return <h3 className="text-xl font-semibold mt-6 mb-2 text-primary">{item.text}</h3>;
  }
  return <p className="text-foreground/90 leading-relaxed">{item.text}</p>;
};

export default function ResourcePage({ params }: { params: { subject: string } }) {
            ...
            <Card className="max-w-4xl mx-auto animate-fade-in bg-card/80 backdrop-blur-sm">
              ...
              <CardContent className="space-y-4">
                 <Alert>
                  ...
                  <AlertDescription>
                    This is an AI-generated reviewer. While helpful, it's recommended to consult official and comprehensive study materials for the best preparation.
                  </AlertDescription>
                </Alert>
